...
export default {
    ...
    beforeCreate() {
        ...
        localStorage.setItem('navMsg', 'Zones d\'activité ')
    },
    created() {
        let CodSte = localStorage.CodUsr
        let Token = localStorage.Token
        let ListSte = localStorage.ListeSte 
        let serverName = localStorage.serverName
        this.$http.post(serverName + '/include/ressources/data/get-Matrix.php?Token=' + Token + '&CodSte=' + CodSte + '&ListSte=' + ListSte + '&node=root',          
        ).then(function(result){
            this.parentSte = result.body[0]            
